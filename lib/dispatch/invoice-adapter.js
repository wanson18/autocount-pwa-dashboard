...
const ISO_DATE_TIME_RE = /^(\d{4}-\d{2}-\d{2})T(?:[01]\d|2[0-3]):[0-5]\d:[0-5]\d(?:\.\d+)?(?:Z|[+-](?:0\d|1[0-4]):?[0-5]\d)?$/;
...
function cancellationFlag(value) {
...
function normalizeDocumentDate(value) {
  const text = requiredText(value, 'docDate');
  const date = ISO_DATE_RE.test(text) ? text : ISO_DATE_TIME_RE.exec(text)?.[1];
  if (!date || !validIsoDate(date)) throw new InvoiceDataError('docDate is invalid');
  return date;
}
...
function normalizeInvoice(row, company) {
  ...
  const docDate = normalizeDocumentDate(master.docDate);
  const cancellationState = master.cancelled ?? master.isCancelled;
  const cancelled = cancellationFlag(cancellationState);
  ...
  const items = details.map((detail) => {
    ...
    const sourceUom = typeof detail.unit === 'string' && detail.unit.trim()
      ? detail.unit
      : detail.uom;
    const uom = typeof sourceUom === 'string' && sourceUom.trim() ? sourceUom.trim() : null;
...
class InvoiceAdapter {
  ...
  async listInvoices(company, startDate, endDate, { includeCancelled = false } = {}) {
    const invoices = [];
    // Malformed source rows are quarantined per row (with a visible count)
    // instead of failing the whole company feed. Only structurally sound
    // rows count toward duplicate detection.
    const quarantined = [];
    ...
    while (totalCount === null || processed < totalCount) {
      ...
      if (payload.data.length === 0) {
        if (processed < totalCount) throw new InvoiceDataError('invoice listing ended before totalCount');
        break;
      }
      ...
      for (const row of payload.data) {
        ...
        let invoice;
        try {
          invoice = normalizeInvoice(row, company);
        } catch (error) {
          if (!(error instanceof InvoiceDataError)) throw error;
          quarantined.push({
            companyKey: company.companyKey,
            docNo: typeof row?.master?.docNo === 'string' ? row.master.docNo : null,
        ...
        if (includeCancelled || !invoice.cancelled) {
          await this._enrichMissingUom(invoice, company);
          invoices.push(invoice);
        }
      }
      if (processed > totalCount) throw new InvoiceDataError('invoice listing exceeded totalCount');
      if (processed >= totalCount) break;
      if (page >= 1000) throw new InvoiceDataError('invoice listing exceeded page limit');
      page += 1;
    }
    invoices.quarantined = quarantined;
    return invoices;
  }

  async getInvoice(company, invoiceId, docDate) {
    const invoices = await this.listInvoices(company, docDate, docDate, {
