...
const SAFE_MESSAGES = Object.freeze({
  ...
  authentication_unavailable: 'Sign-in is temporarily unavailable. Try again later.',
  too_many_requests: 'Too many sign-in attempts. Try again later.',
  resource_conflict: 'A resource with that identity already exists.',
  resource_not_found: 'The requested resource was not found.',
  idempotency_conflict: 'The request ID was already used for a different operation.',
  invoice_cancelled: 'The invoice cannot be assigned because it is cancelled.',
  invoice_missing_uom: 'The invoice has a line without an authoritative unit.',
  invoice_already_assigned: 'The invoice is already assigned.',
  invalid_transition: 'The requested status transition is not allowed.',
  stale_trip: 'The trip changed. Refresh and try again.',
  source_unavailable: 'The invoice source is temporarily unavailable.',
  not_found: 'The requested dispatch route was not found.',
...
function sendCaughtError(res, error) {
  ...
  if (error?.code === 'dispatch_auth_store_unavailable') {
    return sendError(res, 503, 'authentication_unavailable');
  }
  if (error?.code === 'too_many_requests') {
    return sendError(res, 429, 'too_many_requests', undefined, {
      'Retry-After': String(error.retryAfterSeconds || 1),
    });
  }
  if (error?.code === 'invalid_request') return sendError(res, 400, 'invalid_request');
  ...
  if (error?.code === 'idempotency_conflict') return sendError(res, 409, 'idempotency_conflict');
  if (error?.code === 'invoice_cancelled') return sendError(res, 409, 'invoice_cancelled');
  if (error?.code === 'invoice_missing_uom') return sendError(res, 422, 'invoice_missing_uom');
  if (error?.code === 'invoice_already_assigned') return sendError(res, 409, 'invoice_already_assigned');
  if (error?.code === 'invalid_transition') return sendError(res, 409, 'invalid_transition');
  if (error?.code === 'stale_trip') return sendError(res, 409, 'stale_trip');
  if (error?.code === 'source_unavailable') return sendError(res, 503, 'source_unavailable');
