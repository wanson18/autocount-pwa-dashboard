...
function companyConfigOf(configs, companyKey) {
  const config = configs?.[companyKey];
  if (!config || typeof config !== 'object' || Array.isArray(config)
    || config.companyKey !== companyKey
    || typeof config.accountBookId !== 'string' || !config.accountBookId.trim()
...
class DispatchService {
  ...
  async _loadAuthoritativeInvoice(companyKey, invoiceId, docNo, docDate) {
    ...
    if (!company) throw serviceError('source_unavailable');
    let raw;
    try {
