...
class DispatchRepository {
  ...
  async createDriver({ name, licenseNo, phone = null, active = true, actor = null }) {
    try {
  ...
  async createVehicle({ registrationNo, description = null, active = true, actor = null }) {
    try {
      return await withTransaction(this.pool, async (client) => {
        const result = await client.query(`
  ...
  async updateDriver(id, { name, licenseNo, phone, active, actor = null } = {}) {
    const fields = [];
    const values = [];
    ...
    values.push(id);
    const eventType = active === undefined
      ? 'driver_updated'
      : active ? 'driver_reactivated' : 'driver_deactivated';
    try {
      return await withTransaction(this.pool, async (client) => {
        const result = await client.query(`
  ...
  async updateVehicle(id, { registrationNo, description, active, actor = null } = {}) {
    const fields = [];
    const values = [];
    for (const [column, value] of Object.entries({
    ...
    const eventType = active === undefined
      ? 'lorry_updated'
      : active ? 'lorry_reactivated' : 'lorry_deactivated';
    try {
      return await withTransaction(this.pool, async (client) => {
        const result = await client.query(`
