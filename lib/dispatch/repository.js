...
const DEFAULT_DISPATCH_REPORT_LIMIT = 1000;
...
function reportReadLimit(value = DEFAULT_DISPATCH_REPORT_LIMIT) {
  const limit = Number(value);
  if (!Number.isSafeInteger(limit) || limit < 1 || limit > DEFAULT_DISPATCH_REPORT_LIMIT) {
    throw dispatchMutationError('invalid_request');
  }
  return limit;
}
...
class DispatchRepository {
  ...
  async listReportRecords({
    ...
    vehicleId = null,
    status = null,
    limit = DEFAULT_DISPATCH_REPORT_LIMIT,
  } = {}) {
    const filters = [];
    const values = [];
    const boundedLimit = reportReadLimit(limit);
    ...
    if (company !== undefined && company !== null && company !== 'all') {
      values.push(company);
      filters.push(`assignments.company_key = $${values.length}`);
    }
    if (driverId !== undefined && driverId !== null) {
      values.push(driverId);
      filters.push(`trips.driver_id = $${values.length}`);
    }
    if (vehicleId !== undefined && vehicleId !== null) {
      values.push(vehicleId);
      filters.push(`trips.vehicle_id = $${values.length}`);
    }
    if (status !== undefined && status !== null) {
      values.push(status);
      filters.push(`(trips.status = $${values.length} OR assignments.status = $${values.length})`);
    }
    const result = await this.db.query(`
      SELECT
        assignments.id AS assignment_id,
        assignments.trip_id,
        assignments.company_key,
        assignments.invoice_id,
        assignments.doc_no,
        assignments.doc_date,
        assignments.invoice_header,
      ...
      JOIN delivery_trips trips ON trips.id = assignments.trip_id
      JOIN dispatch_drivers drivers ON drivers.id = trips.driver_id
      JOIN dispatch_vehicles vehicles ON vehicles.id = trips.vehicle_id
      ${filters.length ? `WHERE ${filters.join(' AND ')}` : ''}
      ORDER BY trips.trip_date, assignments.id
      LIMIT $${values.length + 1}
    `, [...values, boundedLimit]);
  ...
  async listReports(options) {
    return this.listReportRecords(options);
  }
  ...
  async moveAssignmentMutation({
    ...
    return withDispatchMutationTransaction(this.pool, async (client) => {
      ...
      if (fromTrip.status === 'dispatched' || fromTrip.status === 'completed' || fromTrip.status === 'cancelled'
        || toTrip.status === 'dispatched' || toTrip.status === 'completed' || toTrip.status === 'cancelled') {
        throw dispatchMutationError('invalid_transition');
      }
