...
async function hashDispatchPin(pin, options = {}) {
  ...
  if (!Buffer.isBuffer(salt) || salt.length < MIN_SCRYPT_SALT_BYTES || salt.length > MAX_SCRYPT_SALT_BYTES) {
...
async function authenticateDispatchLogin(
  ...
  if (throttleStore) {
    const decision = await throttleStore.check({ ...bucketKeys, now });
  ...
  if (!user || user.active !== true || !verified) {
    if (throttleStore) await throttleStore.recordFailure({ ...bucketKeys, now });
    return null;
  }
  if (throttleStore) await throttleStore.recordSuccess({ ...bucketKeys, now });
