...
function isPublicAccessEnabled(env = process.env) {
  return typeof env.DISPATCH_PUBLIC_ACCESS === 'string'
    && env.DISPATCH_PUBLIC_ACCESS.trim().toLowerCase() === 'true';
}

function createPublicDispatchSession(now = new Date()) {
...
function getSessionFromRequest(req, options = {}) {
  const session = verifySessionCookie(requestCookieHeader(req), options);
  if (session) return session;
  if (isPublicAccessEnabled(options.env || process.env)) {
    return createPublicDispatchSession(options.now || new Date());
  }
  return null;
}
